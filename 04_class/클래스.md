...
* 자기 자신을 호출하는 매개변수로 ```self``` 키워드 필요. (없을 시 외부에서 호출 불가.)
  ...
  홍길동
  ```
  ※ 일반적으로 Python 에서 클래스를 만들 시 ```__init__``` 메소드만 오버라이딩하여 **객체초기화**에만 이용.
...
* 클래스 속성: ```self.속성```에 할당하는 것이 아니라 class 안에서 바로 할당.
  ...
  일반적으로 Python 의 언어적 특성 때문에, 굳이 생성자에 인수로 받을 속성을 미리 정의할 필요가 없어서,
  클래스 속성은 생략되는 경우가 많음.

* 단, 객체 생성시 인수로 값을 받는 것이 아닌, 기본적으로 공통된 값을 가지는 속성을 정의하고 할 때 사용.
  ...
  ```
  ※ 그러나 함수의 매개변수에 기본값을 입력하는 방식으로 생성자를 정의하면 완전 동일한 내용을 갖게 됨.
  
  ※ 게다가, Python 에는 상수 개념이 없어서 클래스 속성에 접근 및 변경이 불가능한 것이 아니므로,
...
* Python 에서는 함수의 오버로딩(Overloading)이 되지 않음. (마지막에 정의된 함수만 적용.)
