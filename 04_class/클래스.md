...
* 객체 생성시 인수로 값을 받는 것이 아닌, 기본적으로 공통된 값을 가지는 속성을 정의하고 할 때 클래스 속성을 사용.
  ...
  ```
  ※ 그러나 위 예시에서 함수의 매개변수에 기본값을 입력하는 방식으로 생성자를 정의하면 완전 동일한 내용을 갖게 됨.
  
  ※ 게다가, Python 에는 상수 개념이 없어서 클래스 속성에 접근 및 변경이 불가능한 것이 아니므로,
    개발자의 가독성 및 코드 작성의 편의를 제외하면 클래스 속성에 큰 의미는 없음.
  
  ※ 클래스의 상속관계에서 자식 클래스에서 부모 클래스의 값에 접근하고자 할 때 필요.
